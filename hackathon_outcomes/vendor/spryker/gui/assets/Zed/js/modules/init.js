...
var dataTablesSearchDelay = function () {
    ...
    dataTablesWrapper.each(function (index, wrapper) {
            ...
            /*var stateSearchInput = $('<select style="margin-left: 10px !important;" class="form-control input-sm" id="order-state" name="order-state">\n' +
                ...
                '</select>');*/
            var createdFrom = $('<input type="date" style="margin-left: 10px !important;" class="form-control input-sm" id="created-from" name="created-from" value=""/>');
            searchInput.parent().parent().append(createdFrom);
            ...
            if (createdFrom.length && dataTable.length) {
                createdFrom.unbind().bind('input', function (e) {
                    var self = this;
                    clearTimeout(timeoutId);
                    timeoutId = setTimeout(function () {
                        dataTableApi.settings()[0].jqXHR.abort();
...
$(document).ready(function () {
    ...
    $('.gui-table-data').on('preXhr.dt', function (e, settings, data) {
        var createdFrom = $('#created-from').val();
        if (createdFrom !== '-') data.createdFrom = createdFrom;
    });
