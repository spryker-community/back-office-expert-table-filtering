...
class OrdersTable extends AbstractTable
{
    ...
    protected function runQuery(ModelCriteria $query, TableConfiguration $config, $returnRawResults = false)
    {
        ...
        $createdFromSearchValue='';
        if ($this->request->query->has('createdFrom')) {
            $createdFromSearchValue = $this->request->query->get('createdFrom');
        }
        ...
        if (mb_strlen($searchValue) > 0 || $this->isStrictSearch($query, $config) === true) {
            ...
            if($createdFromSearchValue){
//                TODO: Implement search by created date
...
//                    $query->having($gluedCondition);
//                }
//
//                $query->where($gluedCondition);
            }
